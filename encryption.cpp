...
string encrypt (string hash, string nonce, string message) {
    ...
    unsigned char* outputBuffer = (unsigned char*)malloc(sizeof(char)*message.length());
...
string decrypt (string hash, string nonce, string encryption) {
    ...
    cout << "ENCRYPTION LENGTH " << encryption.length()/2 << endl;
    for(int i=0; i < encryption.length()/2; i++){
...
int mainen(){
    string a = "hello this is a super secret message";
    string b = "hellohel";
